...
var next_world_id: usize = 1;

pub fn COMPONENT(world: *world_t, comptime T: type) void {
    if (@sizeOf(T) == 0)
        @compileError("Size of the type must be greater than zero");

    const type_id = getPerTypeGlobalVar(world, T);
    if (type_id != 0) {
        return;
    }

    component_ids_hm.put(type_id, 0) catch @panic("OOM");

    setPerTypeGlobalVar(world, T, ecs_component_init(world, &.{
...
pub fn TAG(world: *world_t, comptime T: type) void {
    ...
    const type_id_ptr = getPerTypeGlobalVar(world, T);
    if (type_id_ptr != 0)
        return;

    var type_id = ecs_entity_init(world, &.{ .name = typeName(T) });
    setPerTypeGlobalVar(world, T, type_id);
}
...
pub fn getSingleton(world: *const world_t, comptime T: type) ?*const T {
    return get(world, id(world, T), T);
}

pub fn setSingleton(world: *world_t, comptime T: type, value: T) void {
    COMPONENT(world, T);
    _ = set(world, id(world, T), T, value);
}
...
pub inline fn id(world: *const world_t, comptime T: type) id_t {
    return getPerTypeGlobalVar(world, T);
}
...
fn TypeMapPair(
    ...
    const export_name = "ECS_TYPES" ++ type_id;
    // @compileLog(export_name ++ " TypeName => " ++ type_name);
    // const type_id = type_name;
    return struct {
        const Ptr = TPtr(Value);
        comptime {
            @export(getTypeMapValue, .{
                .name = export_name,
                .linkage = .Strong,
                .section = section_prefix ++ "ECS_TYPES" ++ "$a",
            });
        }
        var _value: Value = undefined;
        fn getTypeMapValue() callconv(.C) Value {
            return _value;
        }
...
pub const ComponentsIdsLookup = struct {
    const PerWorldTypeIdTuple = struct {
        world: ?usize,
        type_id: id_t,
    };
    ...
    fn perWorldTypesTuple(comptime T: anytype) *[MaxWorlds]PerWorldTypeIdTuple {
        const PerTypeLookup = struct {
            var smth: *T = undefined;
            var tuple: [MaxWorlds]PerWorldTypeIdTuple = [_]PerWorldTypeIdTuple{
                .{
                    .world = null,
                    .type_id = 0,
                },
        ...
        return &PerTypeLookup.tuple;
    }

    pub fn getTypeIdPtr(world: *const world_t, comptime T: anytype) *id_t {
        ...
        for (tuples) |*tuple| {
            if (tuple.world == world_id) {
                return &tuple.type_id;
            } else if (tuple.world == null) {
                best_available_slot = tuple;
            }
        }

        std.debug.assert(best_available_slot != null);
        best_available_slot.?.world = world_id;
        best_available_slot.?.type_id = 0;
        return &best_available_slot.?.type_id;
    }
};

pub fn getPerTypeGlobalVar(world: *const world_t, comptime T: anytype) id_t {
    var type_id_ptr = ComponentsIdsLookup.getTypeIdPtr(world, T);
    return type_id_ptr.*;
}

pub fn setPerTypeGlobalVar(world: *world_t, comptime T: anytype, value: id_t) void {
    var type_id_ptr = ComponentsIdsLookup.getTypeIdPtr(world, T);
    type_id_ptr.* = value;
}
...
pub const os = struct {
    ...
    pub extern var ecs_os_api: api_t;
