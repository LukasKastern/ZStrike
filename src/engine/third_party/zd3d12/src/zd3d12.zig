...
pub const GraphicsContext = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, window: w32.HWND) GraphicsContext {
        ...
        var present_interval: w32.UINT = 1;
        {
            ...
            if (hr == w32.S_OK and allow_tearing == w32.TRUE) {
                present_flags.ALLOW_TEARING = false;
            }
        }
        ...
        const d2d_state = if (enable_d2d) blk_d2d: {
            const dx11 = blk: {
                ...
                hrPanicOnFail(d3d11on12.D3D11On12CreateDevice(
                    @as(*w32.IUnknown, @ptrCast(device)),
                    .{ .DEBUG = enable_debug_layer, .BGRA_SUPPORT = true },
                    null,
                    0,
                    &[_]*w32.IUnknown{@as(*w32.IUnknown, @ptrCast(cmdqueue))},
                    1,
                    0,
            ...
            const swapbuffers11 = blk: {
                ...
                for (swapbuffers11, 0..) |_, buffer_index| {
                    hrPanicOnFail(device11on12.CreateWrappedResource(
                        @as(
                            *w32.IUnknown,
                            @ptrCast(resource_pool.lookupResource(swapchain_buffers[buffer_index]).?.raw.?),
                        ),
    ...
    pub fn createGraphicsShaderPipelineRsVsGsPsFromMemory(
        gctx: *GraphicsContext,
        pso_desc: *d3d12.GRAPHICS_PIPELINE_STATE_DESC,
        vs_cso_mem: []const u8,
        ps_cso_mem: []const u8,
    ) PipelineHandle {
        pso_desc.VS = .{ .pShaderBytecode = vs_cso_mem.ptr, .BytecodeLength = vs_cso_mem.len };
        pso_desc.PS = .{ .pShaderBytecode = ps_cso_mem.ptr, .BytecodeLength = ps_cso_mem.len };
    ...
    pub fn allocateUploadMemory(
        ...
        return .{
            .cpu_slice = @alignCast(std.mem.bytesAsSlice(T, memory.cpu_slice.?)),
    ...
    pub fn createAndUploadTex2dFromDdsMemory(
        gctx: *GraphicsContext,
        memory: []const u8,
        arena: std.mem.Allocator,
        args: struct {
        ...
        const dds_info = try dds_loader.loadTextureFromMemory(memory, arena, gctx.device, 0, &subresources);
        assert(dds_info.resource_dimension == .TEXTURE2D);
        assert(dds_info.cubemap == args.is_cubemap);
