...
pub fn loadTextureFromMemory(file_data: []const u8, arena: std.mem.Allocator, device: *d3d12.IDevice9, max_size: u32, resources: *std.ArrayList(d3d12.SUBRESOURCE_DATA)) !DdsImageInfo {
    if (file_data.len > std.math.maxInt(u32)) {
        return DdsError.InvalidDDSData;
    }

    // Create a stream
    var stream = std.io.StreamSource{ .const_buffer = std.io.fixedBufferStream(file_data) };
