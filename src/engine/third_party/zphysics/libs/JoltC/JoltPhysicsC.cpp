...
FN(toJpc)(const JPH::CharacterVirtual *in) {
    assert(in); return reinterpret_cast<const JPC_CharacterVirtual *>(in);
}

FN(toJpc)(JPH::CharacterVirtualSettings *in) {
    assert(in); return reinterpret_cast<JPC_CharacterVirtualSettings*>(in);
}

FN(toJpc)(JPH::CharacterVirtual *in) {
    assert(in); return reinterpret_cast<JPC_CharacterVirtual *>(in);
}
...
JPC_CharacterVirtual_Create(const JPC_CharacterVirtualSettings *in_settings, const JPC_Real in_position[3], const float in_rotation[4], JPC_PhysicsSystem *in_system) 
{
    auto jph_settings = toJph(in_settings);
    auto character = new JPH::CharacterVirtual(jph_settings, loadRVec3(in_position), JPH::Quat(loadVec4(in_rotation)), toJph(in_system));
    character->AddRef();
    return toJpc(character);
}

JPC_API void 
JPC_CharacterVirtual_Release(JPC_CharacterVirtual *in_character) {
    toJph(in_character)->Release();
}
