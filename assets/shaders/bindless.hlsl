...
void vsMeshPbr(
    ...
    float4x4 object_to_world: _InstanceObjectToWorld,
    uint InstanceID : SV_InstanceID,
    int4 texture_slots: _TextureSlots, 
    ...
    out int4 out_texture_slots: _TextureSlots) {
    const float4x4 object_to_clip = mul(object_to_world, scene_const.world_to_clip);
        
    out_position_clip = mul(float4(position, 1.0), object_to_clip);
    out_position = mul(position, (float3x3)object_to_world);
    out_normal = normal;
    out_tangent = tangent;
    out_texcoords0 = texcoords0;
    out_texture_slots = texture_slots;
}
...
void psMeshPbr(
    ...
    Texture2D srv_base_color_texture = ResourceDescriptorHeap[texture_slots[0]];
    ...
    // out_color = float4(texture_slots[0], 0, 0, 1);
...
[RootSignature(root_signature)]
void psMeshPbrPrototype(
    float4 position_clip : SV_Position,
    float3 position : _Position,
    nointerpolation float3 normal : _Normal,
    float2 texcoords0 : _Texcoords0,
    float4 tangent : _Tangent,
    int4 texture_slots: _TextureSlots,
    out float4 out_color : SV_Target0
) {
    float3 triW_one = abs(normal);
    float3 twiW_two = triW_one / (triW_one.x + triW_one.y + triW_one.z); 

    Texture2D srv_base_color_texture = ResourceDescriptorHeap[texture_slots[0]];

	float2 triUVx = position.zy;
	float2 triUVy = position.xz;
	float2 triUVz = position.xy;

	float2 uv_front = position.xy;
	float2 uv_side = position.zy;
	float2 uv_top = position.xz;    

    uv_top.y = 1 - uv_top.y;
    uv_front.y = 1 - uv_front.y;
    uv_side.y = 1 - uv_side.y;

    if(normal.z < 0) {
        uv_front.x = -uv_front.x;
    }
    ...
    float4 col_front = srv_base_color_texture.Sample(sam_aniso, uv_front);
    float4 col_side = srv_base_color_texture.Sample(sam_aniso, uv_side);
    float4 col_top = srv_base_color_texture.Sample(sam_aniso, uv_top);
...
weights = weights / (weights.x + weights.y + weights.z);
    ...
    // float2 coord = float2(twiW_two);

    // out_color.rgb = normal * 0.5 + 0.5;//srv_base_color_texture.Sample(sam_aniso, coord);
    // out_color.a = 1.0;

    out_color = col;
    out_color.rgb = ApplySRGBCurve(out_color.rgb);
    // out_color.rgb = albedoX * twiW_two.x + albedoY * twiW_two.y + albedoZ * twiW_two.z;
    // out_color.a = 1.0;
}

[RootSignature(root_signature)]
void vsMeshPbrPrototype(
    float3 position : POSITION,
    float3 normal : _Normal,
    float2 texcoords0 : _Texcoords0,
    float4 tangent : _Tangent,
    ...
    float3 worldNormal = mul(normal, (float3x3)object_to_world);

    out_normal = normalize(worldNormal);
    out_tangent = tangent;
